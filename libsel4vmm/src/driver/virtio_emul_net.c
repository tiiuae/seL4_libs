...
static void emul_rx_complete(void *iface, unsigned int num_bufs, void **cookies, unsigned int *lens) {
    ...
    if (idx != guest_idx) {
        ...
        int current_buf = -1;
        uint16_t desc_head = ring_avail(&emul->internal->guest_vspace, vring, idx);
        ...
        ring_used_add(&emul->internal->guest_vspace, vring, used_elem);
...
static void emul_tx_complete(void *iface, void *cookie) {
    ...
    ring_used_add(&emul->internal->guest_vspace, &net->vring[TX_QUEUE], used_elem);
    free(tx_cookie);
